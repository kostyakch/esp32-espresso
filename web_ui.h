...
inline String renderHomePage() {
  String html =
    ...
    "</div>"
    "<div class='card'>"
    "<label>Температура (последние 10 минут)</label>"
    "<canvas id='temp_graph' width='400' height='100' style='width:100%;max-width:400px;height:100px;display:block;border-radius:8px;background:#f8f4ee;'></canvas>"
    "</div>"
    "<div class='card'>"
    "<label>Нагрев</label>"
    "<div class='actions'>"
    ...
    "var hist=d.temp_history;if(Array.isArray(hist)&&hist.length>0){"
    "var c=document.getElementById('temp_graph');if(c&&c.getContext){"
    "var ctx=c.getContext('2d');var w=c.width;var h=c.height;var pad=4;"
    "ctx.clearRect(0,0,w,h);"
    "var yMin=Math.min.apply(null,hist);var yMax=Math.max.apply(null,hist);var range=Math.max(30,yMax-yMin+10);var yMid=(yMin+yMax)/2;"
    "var scaleY=(h-2*pad)/range;var scaleX=hist.length>1?(w-2*pad)/(hist.length-1):1;"
    "ctx.strokeStyle='#c46a2f';ctx.lineWidth=2;ctx.beginPath();"
    "ctx.moveTo(pad,h-pad-(hist[0]-yMid+range/2)*scaleY);"
    "for(var i=1;i<hist.length;i++)ctx.lineTo(pad+i*scaleX,h-pad-(hist[i]-yMid+range/2)*scaleY);"
    "ctx.stroke();"
    "}}"
    "}catch(e){}"
    "}"
